  ...
  return (
    <section className="relative overflow-hidden bg-background pb-16 pt-12">
      <div className="container mx-auto flex flex-col items-center px-4 text-center sm:px-6 lg:px-8">
        ...
        <h1 className="mb-4 max-w-3xl text-center font-sans leading-tight tracking-tight text-foreground">
        ...
        <p className="mb-8 max-w-2xl text-center text-lg text-muted-foreground md:text-xl">
          LumiVertex builds the architecture for high-growth firms by converging
          intelligent AI systems with battle-tested strategic frameworks.
        </p>
        ...
        <div className="mt-12 flex w-full max-w-5xl flex-col gap-6 md:flex-row md:gap-8">
          {/* Left: first image only */}
          <div className="w-full md:w-[60%]">
            <div className="relative aspect-4/3 w-full overflow-hidden rounded-2xl md:h-104">
              <Image src="/images/hero-1.jpg" alt="Team working" fill className="object-cover scale-x-[-1]" sizes="(max-width: 768px) 100vw, 55vw" />
            </div>
          </div>
          ...
          <div className="flex w-full flex-col gap-7 items-center justify-center md:w-[40%]">
            <div className="relative aspect-4/3 w-full overflow-hidden rounded-2xl md:h-80">
              <Image src="/images/hero-2.jpg" alt="Collaboration" fill className="object-cover scale-[2]" sizes="(max-width: 768px) 100vw, 40vw" />
            </div>
            <div className="flex flex-row items-center gap-4 text-center">
              {stats.map((stat) => (
                <div key={stat.value}>
                  <p className="font-heading text-2xl font-semibold text-foreground md:text-4xl">{stat.value}</p>
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-8">
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 text-sm font-medium text-muted-foreground">
            {logos.map((logo) => (
              <span key={logo} className="opacity-60 hover:opacity-100 transition-opacity">{logo}</span>
            ))}
          </div>
        </div>
      </div>
